...
class Trainer():
    ...
    def train(self, model, vocab, train_loader, valid_loader_list, loss_type, start_epoch, num_epochs, args, evaluate_every=1000, opt_name="adam",save_every=5):
        """
        Training
        args:
            model: Model object
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            print("TRAIN")
            model.train()
            print(f'loader : {train_loader}, epoch : {epoch}')
            pbar = tqdm(iter(train_loader), leave=True, total=len(train_loader))
            max_len = 0
            print('Pbar:',type(pbar),',iter:',start_iter)

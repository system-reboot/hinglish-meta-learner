...
class MetaTrainer():
    """
    Trainer class for Reptile Meta-Learning
    """
    def __init__(self):
        logging.info("Meta Trainer (Reptile) is initialized")
    ...
    def forward_one_batch(self, model, vocab, src, trg, src_percentages, src_lengths, trg_lengths, smoothing, loss_type, verbose=False, accent_id=None):
        # This method remains largely the same as in the MAML implementation
    ...
    def train(self, model, vocab, train_data_list, valid_data_list, loss_type, start_it, num_it, args, inner_opt=None, outer_opt=None, evaluate_every=1000, window_size=100, last_summary_every=1000, last_metrics=None, early_stop=10, cpu_state_dict=False):
        """
        Reptile Training
        Key differences from MAML:
        1. Simpler meta-update strategy
        2. No second-order derivatives
        3. Direct interpolation between initial and fine-tuned weights
        """
        ...
        # Define batch loader function (same as MAML)
        ...
        while it < num_it:
            ...
            try:
                ...
                for manifest_id in range(len(train_tmp_buffer)):                
                    ...
                    # Reptile Update: Multiple inner loop updates
                    for _ in range(5):  # Number of inner loop updates (can be tuned)
                        # Meta Train
                        model.train()
                    ...
                    # Reptile Meta-Update (interpolation)
                    # This is the key difference from MAML
                    with torch.no_grad():
                        for (name, param), orig_param in zip(model.named_parameters(), weights_original.values()):
                            # Interpolate between original and fine-tuned weights
                            # Reptile uses a simple linear interpolation
                            param.data = orig_param + (param.data - orig_param) * args.meta_lr
                    ...
                    # Update total loss for logging
                    total_loss += val_loss.item()

                    # Reset to original weights for next task
